...
import { Box, Container, Grid, Typography, Chip, CircularProgress, Alert, Snackbar, Avatar, Paper } from '@mui/material';
...
const ItemDetail = () => {
  ...
  return (
    <Box className="page-enter" sx={{ minHeight: '100vh', pt: '110px', pb: 10 }}>
      <Container maxWidth="lg">
        ...
        {/* Back link */}
        <Box className="reveal" component={RouterLink} to="/browse"
        ...
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 4, alignItems: 'flex-start' }}>
          ...
          {/* LEFT — Gallery */}
          <Box className="reveal" sx={{ flex: '1 1 0', minWidth: 0, width: '100%' }}>
            <Box>
              {/* Category + verified */}
              <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                <Chip label={item.category} sx={{ bgcolor: 'var(--bg-card)', color: 'var(--ink)', fontWeight: 600, fontSize: '12.5px', border: '1px solid var(--line)' }} />
                {item.isAvailable && (
              ...
              {/* Main image */}
              <Box sx={{ borderRadius: '20px', overflow: 'hidden', boxShadow: 'var(--shadow)', height: { xs: 260, md: 460 }, width: '100%', bgcolor: 'var(--line)', transition: 'transform 0.35s var(--ease), box-shadow 0.35s var(--ease)', '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 26px 60px rgba(0,0,0,0.42)' } }}>
          ...
          {/* RIGHT — Booking Card */}
          <Box className="reveal" sx={{ flex: '0 0 auto', width: { xs: '100%', md: '320px' }, position: 'sticky', top: '110px' }}>
            <Box sx={{ bgcolor: 'var(--bg-card)', borderRadius: '20px', border: '1px solid var(--line)', boxShadow: 'var(--shadow)', p: 2.5, backgroundImage: 'linear-gradient(180deg, rgba(139,92,246,0.08) 0%, rgba(22,22,30,0.96) 18%, rgba(22,22,30,1) 100%)' }}>
              ...
              <Box onClick={handleBook}
                sx={{
                  ...
                  '&:hover': {
                    ...
                    boxShadow: !bookingLoading ? '0 16px 34px rgba(139,92,246,0.38)' : '0 12px 30px rgba(139,92,246,0.28)',
                    filter: !bookingLoading ? 'brightness(1.05)' : 'none'
                  },
                  mb: 1
                }}
              >
                ...
                <Box sx={{ display: 'flex', gap: 1, alignItems: 'stretch' }}>
                  {priceTiers.map((tier) => (
                    <Box key={tier.days} onClick={() => applyTier(tier.days)}
                      ...
                      <Typography sx={{ fontFamily: '"Fraunces", serif', fontWeight: 700, fontSize: '0.95rem', color: 'var(--ink)', lineHeight: 1.1 }}>Rs {tier.total.toLocaleString()}</Typography>
                      <Typography sx={{ fontSize: '11px', color: 'var(--slate)', mt: 0.45, whiteSpace: 'nowrap' }}>{tier.days} day{tier.days > 1 ? 's' : ''}</Typography>
                    </Box>
                  ))}
                </Box>
              </Box>
        ...
        {/* ── ROW 2: Owner + Map ── */}
        <Box className="reveal" sx={{ mt: 4, bgcolor: 'var(--bg-card)', border: '1px solid var(--line)', borderRadius: '18px', overflow: 'hidden' }}>
          <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' } }}>
            ...
            {/* RIGHT half — Map */}
            <Box sx={{ flex: '1 1 50%', minWidth: 0 }}>
              <Box sx={{ height: { xs: 260, md: '100%' }, minHeight: 300, position: 'relative', overflow: 'hidden', bgcolor: 'var(--bg)' }}>
                <Box
                  component="iframe"
        ...
        <Box className="reveal" sx={{ mt: 6 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', mb: 3, flexWrap: 'wrap', gap: 2 }}>
            <Box>
              <Typography sx={{ fontFamily: '"Fraunces", serif', fontWeight: 600, fontSize: '1.4rem', color: 'var(--ink)', mb: 0.5 }}>
                Reviews
              </Typography>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                {avgRating && (
                  <Typography sx={{ fontWeight: 700, fontSize: '15px', color: 'var(--ink)' }}>
                    <Box component="span" sx={{ color: 'var(--amber)', mr: 0.3 }}>★</Box>
                    {avgRating}
                  </Typography>
                )}
            ...
            {reviewsLoading && (
              <CircularProgress size={18} sx={{ color: 'var(--purple)' }} />
            )}
          </Box>
          ...
          {reviewsLoading ? (
            ...
            <Paper elevation={0} sx={{
              ...
              <Typography sx={{ fontSize: '36px', mb: 1.5 }}>⭐</Typography>
              <Typography sx={{ color: 'var(--ink)', fontWeight: 600, fontSize: '15px', mb: 0.5 }}>
                No reviews yet
              </Typography>
              <Typography sx={{ color: 'var(--slate)', fontSize: '13px' }}>
                Be the first to review this item after your rental.
              </Typography>
            </Paper>
          ) : (
            <Grid container spacing={2.5}>
              {reviews.map((r, i) => {
                const renterName = r.renter?.name || 'Anonymous';
                const avatarChar = renterName[0]?.toUpperCase() || '?';
                const ratingStars = Array.from({ length: 5 }, (_, s) => s < (r.rating || 5));
                return (
                  <Grid item xs={12} sm={6} key={r._id || i}
                    sx={{ animation: 'fadeInUp 0.5s ease-out forwards', opacity: 0, animationDelay: `${i * 0.08}s` }}
                  >
                    <Box sx={{
                      bgcolor: 'var(--bg-card)',
                      border: '1px solid var(--line)',
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5 }}>
                        ...
                        <Box sx={{ flex: 1, minWidth: 0 }}>
                          <Typography sx={{ fontWeight: 700, fontSize: '14px', color: 'var(--ink)' }}>{renterName}</Typography>
                          <Typography sx={{ fontSize: '11.5px', color: 'var(--slate)', fontFamily: '"IBM Plex Mono", monospace' }}>
                            {new Date(r.createdAt || Date.now()).toLocaleDateString()}
                          </Typography>
                        </Box>
                        <Box sx={{ display: 'inline-flex', alignItems: 'center' }}>
                          {ratingStars.map((filled, sIdx) => (
                            <Star
                              key={sIdx}
                              size={13}
                              fill={filled ? '#E8A33D' : 'transparent'}
                              color={filled ? '#E8A33D' : 'var(--line)'}
                              strokeWidth={filled ? 0 : 1.5}
                              style={{ marginRight: sIdx === 4 ? 0 : -1 }}
                            />
                          ))}
                        </Box>
                      </Box>
